...
// @LINE:36
def downloadDistant(url:String): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "download/" + implicitly[PathBindable[String]].unbind("url", URLEncoder.encode(url, "utf-8")))
}
...
def downloadDistant : JavascriptReverseRoute = JavascriptReverseRoute(
   ...
   """
      function(url) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "download/" + (""" + implicitly[PathBindable[String]].javascriptUnbind + """)("url", encodeURIComponent(url))})
      }
   """
)
...
// @LINE:36
def downloadDistant(url:String): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.Application.downloadDistant(url), HandlerDef(this, "controllers.Application", "downloadDistant", Seq(classOf[String]), "GET", """""", _prefix + """download/$url<[^/]+>""")
)
