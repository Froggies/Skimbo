...
// @LINE:23
class ReverseLongPolling {
    

// @LINE:23
def connect(): Call = {
   Call("GET", _prefix + { _defaultPrefix } + "api/stream/longpolling")
}
...
// @LINE:23
def connect : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.stream.LongPolling.connect",
   """
      function() {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "api/stream/longpolling"})
      }
   """
)
...
// @LINE:23
def connect(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.stream.LongPolling.connect(), HandlerDef(this, "controllers.stream.LongPolling", "connect", Seq(), "GET", """ Streams -- LongPolling""", _prefix + """api/stream/longpolling""")
)
